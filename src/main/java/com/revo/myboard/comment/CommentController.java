...
public class CommentController {
  ...
  public void createComment(
      @RequestHeader("Authorization") String token, @RequestBody @Valid CreateDTO createDTO,
      HttpServletRequest request) {
    log.info(
        "User with ip: " + request.getRemoteAddr() + " creating comment with details: "
            + createDTO.toString());
  ...
  public void editCommentById(
    ...
    log.info("User with ip: " + request.getRemoteAddr() + " editing comment with id: " + id
        + " with details: "
        + editDTO.toString());
  ...
  @ForUser
  public void deleteCommentByIdAsUser(
      @RequestHeader("Authorization") String token, @PathVariable long id,
      HttpServletRequest request) {
  ...
  @ForUser
  @ResponseStatus(HttpStatus.CREATED)
  public void giveLikeForCommentById(
      @RequestHeader("Authorization") String token, @PathVariable long id,
      HttpServletRequest request) {
    log.info(
        "User with ip: " + request.getRemoteAddr() + " giving like for comment with id: " + id);
    commentService.giveLikeForCommentById(token, id);
    log.info(
        "User with ip: " + request.getRemoteAddr() + " success gived like for comment with id: "
            + id);
  }

  @DeleteMapping("/unlike/{id}")
  @ForUser
  public void removeLikeFromCommentById(
      @RequestHeader("Authorization") String token, @PathVariable long id,
      HttpServletRequest request) {
    log.info(
        "User with ip: " + request.getRemoteAddr() + " removing like from comment with id: " + id);
    commentService.removeLikeFromCommentById(token, id);
    log.info(
        "User with ip: " + request.getRemoteAddr() + " success removed like from comment with id: "
            + id);
  }
